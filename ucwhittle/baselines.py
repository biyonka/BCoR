...
def optimal_policy_current_time(env, n_episodes, n_epochs, discount):
    ...
    for epoch in range(n_epochs):
        ...
        for t in range(1, T + 1):
            ...
            for i in range(N):
                #edited to take in a non-stationary transition
                arm_transitions = true_transitions[i, env.timestep, :, :, 1]
...
def optimal_policy_all_time_avg(env, n_episodes, n_epochs, discount):
    """ optimal policy based on true transition probabilities """
    N         = env.cohort_size
    n_states  = env.number_states
    n_actions = env.all_transitions.shape[2]
    ...
    for epoch in range(n_epochs):
        ...
        for t in range(1, T + 1):
            ...
            min_chosen_subsidy = -1 #0
            for i in range(N):
                #take an average of person i's transitions for each state and action over all time
                arm_transitions = np.mean(true_transitions[i, :, :, :, 1], axis = 0)
                # memoize to speed up
                check_set_val = memoizer.check_set(arm_transitions, state[i])
                if check_set_val != -1:
                    state_WI[i] = check_set_val
...
def optimal_policy_current_time_avg(env, n_episodes, n_epochs, discount):
    """ optimal policy based on true transition probabilities """
    N         = env.cohort_size
    n_states  = env.number_states
    n_actions = env.all_transitions.shape[2]
    ...
    for epoch in range(n_epochs):
        ...
        for t in range(1, T + 1):
            ...
            min_chosen_subsidy = -1 #0
            for i in range(N):
                #take an average of person i's transitions for each state and action up to the current timestep
                arm_transitions = np.mean(true_transitions[i, np.arange(0, (env.timestep+1), 1), :, :, 1], axis = 0)
                #state_WI[i] = arm_compute_whittle(arm_transitions, state[i], discount, min_chosen_subsidy)
                # memoize to speed up
                check_set_val = memoizer.check_set(arm_transitions, state[i])
                if check_set_val != -1:
