...
class RMABSimulator_ns(gym.Env):
    '''
    ...
    def __init__(self, all_population, all_features, all_transitions, cohort_size, episode_len, n_instances, n_episodes, budget,
        ...
        Initialization
        '''
        #when cohort_size  = all_population, does it just run every woman at each reset?
        ...
        #silence this for now, not gonna deal with good state assumptions
    ...
    def step(self, action):
        ...
        for i in range(self.cohort_size):
            #modified to deal with time varying aspect of the non-stationary transitions
            #transitions now a N x T x S x A x S np.array, so need to keep track of the true transition prob's timestep now
            prob = self.transitions[i, self.timestep, self.states[i], action[i], :]
...
def generate_context(transitions):
    ##get matrix that next state is engaging
    #p_next_state_e = transitions[:,:, : ,1]
    # get P_i(1|0, 1)
    p_ne_to_e_from_a_1 = transitions[:,0, 1 ,1]
    # get P_i(1|1, 1)
    p_e_to_e_from_a_1 = transitions[:,1, 1 ,1]
    # get P_i(1|0, 0)
    p_ne_to_e_from_a_0 = transitions[:,0, 0 ,1]
    # get P_i(1|1, 0)
    p_e_to_e_from_a_0 = transitions[:,1, 0 ,1]
    #if a woman is likely to switch to engaging as a result of action
    X1 = np.random.normal(p_ne_to_e_from_a_1, 1)
    #if a woman is likely to stay engaging as a result of action
    X2 = np.random.normal(p_e_to_e_from_a_1, 1)
    #if a woman is likely to switch to engaging without any action at all
    X3 = np.random.binomial(1, p_ne_to_e_from_a_0)
    #X3 = p_ne_to_e_from_a_0 > 0.5
    #if a woman is likely to stay engaging without any action at all
    X4 = np.random.binomial(1, p_e_to_e_from_a_0)
   # X4 = p_e_to_e_from_a_0 > 0.5
    #X = np.sum(transitions[:,:, : ,1], axis =(1, 2))
    return np.array([X1, X2, X3, X4]).transpose()#X.reshape(X.shape[0], 1)
...
if __name__ == '__main__':
    ...
    simulator = RMABSimulator_ns(all_population, all_features, all_transitions, cohort_size, episode_len, budget)
