...
from utils import Memoizer, get_valid_lcb_ucb, get_ucb_conf, get_glm_conf
...
def UCWhittle(env, n_episodes, n_epochs, discount, alpha, method='QP', VERBOSE=False):
    ...
    budget    = env.budget
    T         = env.episode_len * n_episodes #represents H timesteps (episode_len) across n_episodes
    ...
    for epoch in range(n_epochs):
        ...
        #each little t represents a single timestep (h)
        for t in range(1, T + 1):
            ...
            #seems like the action and whittle indices are being updated at each h, not each t; does this match what's written in the paper?
            ...
            #done tracks whether H is completed or not; after H timesteps, the environment will reset w a new initial state
    ...
    #np.save('results/transitions/ucbw_est_p.npy', est_p)    # .npy extension is added if not given
    return all_reward

def TSWhittle(env, iteration, n_episodes, n_epochs, discount, param1, param2, method = 'tswhittle', VERBOSE=False):
    """
    discount = discount factor
    alpha = for confidence radius """
    N         = env.cohort_size
    ...
    for epoch in range(n_epochs):
        ...
        n_pulls  = np.zeros((N, n_states, n_actions))  # number of pulls
        cum_prob = np.zeros((N, n_states, n_actions))  # transition probability estimates for going to ENGAGING state (represents number of times next state was 1 for a given state, action pair and person)
        ...
        for t in range(1, T + 1):
            state = env.observe()
            est_transitions = np.random.beta(cum_prob+param1, n_pulls-cum_prob+param2)

            action, state_WI = ts_whittle_step(est_transitions, state, discount, budget, memoizer)
            next_state, reward, done, _ = env.step(action)
            ...
            next_states=np.append(next_states, next_state)
            wis = np.append(wis, state_WI)
            people=np.append(people, np.arange(1, N+1))
            ...
            #make pandas dataframe that saves state, action, and next state
    tswhit_df = pd.DataFrame({'person':people, 'time':np.repeat(range(1, T+1), N), 'state':states, 'action':actions, 'wis':wis, 'y':next_states, 'method':'tswhittle_py', 'iteration':iteration})
    #np.save('results/transitions/ucbw_est_p.npy', est_p)    # .npy extension is added if not given
    return [all_reward, tswhit_df]
...
def extreme_points_step(est_p, conf_p, state, discount, budget, memoizer):
    ...
    #check dimension of conf_p to determine how to parse
    if conf_p.ndim == 3:
        ...
        est_transitions[:, :, 1] = conf_p[:, :, 1, 1]  # for active action, take UCB
        est_transitions[:, :, 0] = conf_p[:, :, 0, 0]  # for passive action, take LCB
