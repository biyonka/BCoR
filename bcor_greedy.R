...
run_greedy_thompson_sampler_exploration = function(E, true_transitions_df, model, X, B, N, t, k, d,  budget, burn, seed=123){
  ...
  for(current_time in seq(1, t)){
    ...
    if(current_time == 1){
      ...
      b0_HMC = post_params$b0
      b1_HMC = post_params$b1
      ...
      te = sapply(seq(1, N, 1), function(i){
        state = s[i] + 1
        transition_est_a1 = pnorm(X[i,]%*%beta_HMC[state, 2, ] + B[current_time,]%*%eta_HMC[state, 2, ] + alpha_HMC[state, 2,i])
        transition_est_a0 = pnorm(X[i,]%*%beta_HMC[state, 1, ] + B[current_time,]%*%eta_HMC[state, 1, ] + alpha_HMC[state, 1,i])
    ...
    write.csv(df, file = paste0(csv, '/iteration_rewards/rmab_run_E_',E,'.csv'), row.names = FALSE)
  }
  return(list(df))
}


rmab_run = run_greedy_thompson_sampler_exploration(E, true_transitions_df,
                                                   model = model,
                                                   X, B, N, t, k, d, budget, burn = burnin, seed = random_seed)

warnings()

rmab_run_df = data.frame(rmab_run[[1]])


run_greedy_oracle = function(true_transitions_df, N, t, k, d, budget){
  #pre-allocate final data matrix
  df <- matrix(data=NA,nrow=(N*t),ncol=5)
  colnames(df) = c('person', 'time', 'state', 'action', 'y')
  ...
  #wrap this in a loop over time until end of time horizon
  for(current_time in seq(1, t)){
    if(current_time == 1){s = round(runif(N))}else{s = next_state} 
    
    te = sapply(seq(1, N, 1), function(i){
      transition_est_a1 = filter(true_transitions_df, person == i, state==s[i], action==1, time == current_time)$transitions
      transition_est_a0 = filter(true_transitions_df, person == i, state==s[i], action==0, time == current_time)$transitions
      return(transition_est_a1 - transition_est_a0)
    })
    #pull women with budget highest treatment effects
    #get indices of women to pull
    ...
    #add new transition data to growing data table for new t
    df[((N*current_time)- (N-1)):(N*current_time),] = matrix(c(seq(1, N), rep(current_time, N), s, a, next_state), nrow = N, byrow = FALSE)
    write.csv(df, file = paste0(csv, '/iteration_rewards/oracle_run_E_',E,'.csv'), row.names = FALSE)
  }
  return(df)
}

oracle_run = data.frame(run_greedy_oracle(true_transitions_df, N, t, k, d, budget))

#bind oracle and ts runs
full_run = rbind(rmab_run_df, oracle_run)
full_run['Method'] = c(rep('ts_la', nrow(rmab_run_df)), rep('greedy_oracle', nrow(oracle_run)))
full_run['iteration'] = E

# #generate rewards df for this epoch

rewards_df = full_run %>% group_by(Method, time) %>% summarize(reward = sum(y))%>% mutate(cum_reward = cumsum(reward)) %>%
  mutate(time_avg_reward = cumsum(reward)/time)
rewards_df['iteration'] = E
#
# #save full run df and rewards
write.csv(full_run, file = paste0(csv,"/iteration_runs/full_run_E_",E,".csv"),  row.names = FALSE)
